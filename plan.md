...
6. Decide Node.js LTS version (e.g., 20.x) and pin in `.nvmrc`. [done]
7. use npm. [done]
8. Document required secrets (OAuth, NextAuth) in `design/authentication.md` refs. [done]

### Phase 1 — Project scaffolding [COMPLETED]

9. Bootstrap Next.js App Router project with TypeScript. [done]
10. Initialize package manager lockfile (npm or pnpm). [done]
11. Add and configure ESLint with TypeScript recommended rules. [done]
12. Add Prettier and eslint-config-prettier to avoid style conflicts. [done]
13. Add Husky + lint-staged for pre-commit checks. [done]
14. Configure `tsconfig.json` base paths and strict mode. [done]
15. Create `src/` layout; enable `baseUrl` and path aliases. [done]
16. Create `src/app/(site)/` and `src/app/api/` folders. [done]
17. Add an initial home route with a placeholder page. [done]
18. Add a shared `src/styles/theme.ts` for MUI theme. [done]
19. Wire MUI `ThemeProvider` in `src/app/layout.tsx`. [done]
20. Add a global CSS reset and fonts. [done]

### Phase 2 — Dependencies [COMPLETED]

21. Install UI libs: `@mui/material @mui/icons-material @emotion/react @emotion/styled`. [done]
22. Install auth: `next-auth` and `@types/node` updates if needed. [done]
23. Install charts: `recharts`. [done]
24. Install validation/utilities: `zod`, `date-fns`, `uuid`. [done]
25. Install CSV/ZIP tooling: `papaparse`, `csv-parse`, `csv-stringify`, `jszip`. [done]
26. Install Google API SDK: `googleapis`. [done]
27. Install rate-limiter: lightweight in-memory or `lru-cache`. [done]
28. Install test tooling: `vitest` + `@vitest/ui`, `@testing-library/react`, `playwright`. [done]
